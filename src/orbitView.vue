<template>
  <div id="3d-orbit" class="orbit" ref="orbitElement"></div>
</template>
...
let createEarth = (radius, segments) =>
  new THREE.Mesh(
...
let createMoon = (radius, segments) =>
  new THREE.Mesh(
    new THREE.SphereGeometry(radius, segments, segments),
    new THREE.MeshPhongMaterial({
...
let createCraft = (radius, segments, color) =>
  new THREE.Mesh(
    new THREE.SphereGeometry(radius, segments, segments),
...
export default {
  ...
  mounted() {
    ...
    const refreshCrafts = function (orbits) {
      ...
      orbitElements = [];
      for (let orbit of orbits) {
        if (orbit.name && orbit.name.toLowerCase() == "moon") {
    ...
    const setOrbits = function ({ orbits, currentTime }) {
      currentTime += 11*60*60
      time = currentTime;
      earth.rotation.y = (currentTime * earthRotationConst);
      clouds.rotation.y = (1.1 * currentTime * earthRotationConst);
    ...
    let updateCanvasSize = function () {
      let width = self.$refs.orbitElement.clientWidth;
      let height = self.$refs.orbitElement.clientHeight;
